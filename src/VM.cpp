    ...
    void VM::run()
    {
        while (ip < code.size())
        {
            ...
            case Opcode::GETFIELD:
            {
                ...
                DBG("GETFIELD, requested index: " << (int)fieldIndex << ", fieldCount: " << cls->fields.size());
                ...
                push(value);
                DBG("GETFIELD from ObjRef " << objRef << " (" << cls->name << "." << field.name << "), Value = " << value);
                break;
                // Code added by Mokshith - End
            ...
            case Opcode::PUTFIELD:
            {
                ...
                *reinterpret_cast<int32_t*>(baseAddress + offset) = value;
                DBG("PUTFIELD on ObjRef " << objRef << " (" << cls->name << "." << field.name << "), Value = " << value);
                break;
                // Code added by Mokshith - End
            }
