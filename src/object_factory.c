...
bool object_factory_register_class(ObjectFactory_t* factory, const ClassInfo_t* cls) {
    ...
    int current_class_index = hash_map_size(&factory->classes) - 1; // This is problematic if keys are removed
    string_t class_name_copy;
    string_init_from_cstr(&class_name_copy, string_cstr(&cls->name));
    if (!hash_map_put(&factory->class_offset_to_name, (string_t*)&current_class_index, &class_name_copy)) { // Key is int, value is string_t
